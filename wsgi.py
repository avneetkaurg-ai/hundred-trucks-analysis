...
def send_report():
    try:
        ...
        for vno in live_trucks:
            ...
            # get route info from enroute_trips
            con = get_db(); cur = con.cursor()
            cur.execute('SELECT pickup_pin, drop_pin, pickup_city FROM enroute_trips WHERE truck_no=%s', (vno,))
            row = cur.fetchone()
            con.close()
            pickup_city = '—'
            drop_city = '—'
            if row:
                _, drop_pin_val, pcity = row[0], row[1], row[2] if len(row) > 2 else None
                if pcity:
                    pickup_city = pcity
                if drop_pin_val:
                    dlat, dlng, dcity = resolve_pincode(drop_pin_val)
                    if dcity:
                        drop_city = dcity
            msg = (
                f"Hey there!\n\n"
