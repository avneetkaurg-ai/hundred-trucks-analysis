...
def send_report():
    try:
        ...
        # Message 1: summary
        summary = f"Daily Update – {today}\nLive trucks: {len(live_trucks)}\nCompleted today: {len(completed_trucks)}"
        ...
        for vno in live_trucks:
            ...
            con = get_db(); cur = con.cursor()
            cur.execute('SELECT pickup_pin, drop_pin FROM enroute_trips WHERE truck_no=%s', (vno,))
            row = cur.fetchone()
            con.close()
            pickup_city = '—'
            drop_city = '—'
            if row:
                pickup_pin_val, drop_pin_val = row[0], row[1]
                if pickup_pin_val:
                    _, _, pcity = resolve_pincode(pickup_pin_val)
                    if pcity: pickup_city = pcity
                if drop_pin_val:
                    _, _, dcity = resolve_pincode(drop_pin_val)
                    if dcity: drop_city = dcity
            msg = (
                f"Hey there!\n\n"
                f"Your shipment is on its way.\n\n"
                f"*Truck No:* {vno}\n"
                f"*Route:* {pickup_city} → {drop_city}\n\n"
                f"Track your live route here:\n"
                f"{BASE_URL}/track.html?token={token}\n\n"
                f"_For any queries, feel free to reach out._\n\n"
                f"Have a nice day!\n*Mirchi-Lime*"
            )
