...
def send_report_now():
    try:
        payload = json.dumps({
            'from': 'Hundred Trucks <onboarding@resend.dev>',
            'to': [REPORT_TO],
            'subject': 'Test Email from Hundred Trucks',
            'html': '<p>This is a test email from the Hundred Trucks dashboard.</p>'
        }).encode()
        ...
        print(f'Test email sent to {REPORT_TO}: {result}', flush=True)
        return jsonify({'status': 'sent', 'to': REPORT_TO, 'result': result})
    except Exception as e:
