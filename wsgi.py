...
RESEND_KEY   = os.environ.get('RESEND_KEY', 're_2CitAsmz_NtqF5s1R9SkZbPkHRjGcBUGx')
...
def send_report_now():
    try:
        import smtplib
        from email.mime.text import MIMEText
        msg = MIMEText('<p>Test email from Hundred Trucks dashboard.</p>', 'html')
        msg['Subject'] = 'Test - Hundred Trucks'
        msg['From']    = GMAIL_USER
        msg['To']      = REPORT_TO
        with smtplib.SMTP_SSL('smtp.gmail.com', 465) as s:
            s.login(GMAIL_USER, GMAIL_PASS)
            s.sendmail(GMAIL_USER, REPORT_TO, msg.as_string())
        print('Test email sent to ' + REPORT_TO, flush=True)
        return jsonify({'status': 'sent', 'to': REPORT_TO})
    except Exception as e:
        print('Test email ERROR: ' + str(e), flush=True)
        return jsonify({'status': 'error', 'error': str(e)})
...
if not os.environ.get('WERKZEUG_RUN_MAIN'):
    ...
    scheduler.add_job(ping_credit,        'cron', hour=12, minute=30)           # 12:30 PM IST
    scheduler.add_job(ping_credit,        'cron', hour=17, minute=30)           # 5:30 PM IST
    scheduler.add_job(ping_credit,        'cron', hour=23, minute=30)           # 11:30 PM IST
    scheduler.add_job(sync_credit_trips,  'cron', hour=0,  minute=0)            # midnight IST
    scheduler.add_job(sync_enroute_trips, 'cron', hour=0,  minute=0)            # midnight IST
    scheduler.add_job(send_morning_report,'cron', hour=6,  minute=30)           # 6:30 AM IST
    scheduler.start()
